...
function publicPosts(posts, overrides = {}) {
  return posts.filter(post => !post.private && !post.password).map(post => {
    ...
    return { file, title: display.title || post.title || file, originalTitle: post.title || '',
      ...
      summary: display.summary || plainText(post.description) || text.slice(0, 110), ...readingStats(text),
...
module.exports = { escape, plainText, readingStats, publicPosts, safeUrl, withHeadingAnchors, seriesPosts };
