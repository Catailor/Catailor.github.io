...
function library(root) {
  ...
  if (!fs.existsSync(file)) {
    ...
    const series = (config.series || []).map(item => ({id:item.id, title:item.title, description:item.description || '', favorite:false,
      posts: item.category ? all.filter(p => { const matter = require('hexo-front-matter').parse(fs.readFileSync(safePath(root,p.id),'utf8')); return [].concat(matter.categories || []).includes(item.category); }).sort((a,b) => item.order === 'oldest' ? a.date.localeCompare(b.date) : b.date.localeCompare(a.date)).map(p => basename(p.id)) : item.posts || []}));
    const templates = [
...
function validateLibrary(value, available) {
  ...
  const validate = (items, template) => {
    ...
    return items.map(item => {
      ...
      return {...base,description:item.description,posts:item.posts};
    });
  };
...
function publicConfig(baseYaml, local, eligible) {
  ...
  config.series = local.series.map(s => ({id:s.id,title:s.title,description:s.description,posts:s.posts.filter(name => eligible.has(name))})).filter(s => s.posts.length || existing.has(s.id));
